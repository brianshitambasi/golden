    ...
    <div className="bg-white rounded-4 shadow-sm p-4">
      ...
      <div className="d-flex gap-3 border-bottom pb-2 mb-4">
        {['Unpaid', 'To be Shipped', 'Shipped', 'To be Reviewed', 'Return/Refund'].map((tab) => (
          <button key={tab} className="btn btn-sm text-muted">{tab}</button>
        ))}
      </div>
      <div className="bg-warning bg-opacity-10 rounded-3 p-3 mb-4">
        <div className="d-flex justify-content-between align-items-center">
          <div>
            ...
            <small>Sponsored by Smartrahalfe</small>
          </div>
      ...
      <div className="mb-4">
        ...
        <div className="d-flex gap-4">
          <div className="text-center"><div className="bg-light rounded-circle p-2 mb-1"><i className="fas fa-wallet text-warning"></i></div><small>Wallet</small></div>
          <div className="text-center"><div className="bg-light rounded-circle p-2 mb-1"><i className="fas fa-ticket-alt text-warning"></i></div><small>Vouchers</small></div>
          <div className="text-center"><div className="bg-light rounded-circle p-2 mb-1"><i className="fas fa-coins text-warning"></i></div><small>Coins</small></div>
        </div>
      </div>
      <div>
        <h6 className="fw-bold mb-3">My Services</h6>
        <div className="row g-3">
          {['Address Book', 'My Reviews', 'Sell on Kilimall', 'FAQ', 'Customer Service', 'Setting'].map((s) => (
            <div key={s} className="col-6"><button className="btn btn-outline-secondary w-100 text-start rounded-3"><i className="fas fa-circle-info me-2 text-warning"></i><small>{s}</small></button></div>
          ))}
        </div>
      </div>
    </div>
  ...
  const CheckoutContent = () => (
    <>
      <div className="bg-white rounded-4 shadow-sm p-4 mb-4">
        ...
        <div className="d-flex justify-content-between mb-3"><span>Delivery</span><span>KES {deliveryFee.toLocaleString()}</span></div>
        <hr />
        <div className="d-flex justify-content-between fs-5 fw-bold"><span>Total</span><span className="text-success">KES {total.toLocaleString()}</span></div>
      </div>
      <div className="bg-white rounded-4 shadow-sm p-4">
        ...
        {processing && paymentStatus === 'pending' && (
          <div className="text-center py-4">
            ...
            <p className="text-muted small">
              Complete via{' '}
              <a href="https://developer.safaricom.co.ke/simulator" target="_blank" rel="noopener noreferrer">
                Daraja Simulator
              </a>
            </p>
          </div>
